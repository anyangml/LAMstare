        ...
        for ls in sys:
            for frame in ls:
                ...
                if not np.isnan(energy_predict):
                    ...
                    write("/mnt/data_nas/penganyang/ani_cif.cif",atoms)
                    break
...
def main(model_name, testpath_mapping):
    ...
    if model_name == "MACE":
        ...
        CALC = DP("/mnt/data_nas/public/multitask/training_exps/1126_prod_shareft_120GUP_240by3_single_384_96_24/frezmptrj800w.pth")
    elif model_name == "Orb":
    ...
    for ood_name, testpath in mapping.items():
        ...
        if "Force MAE" not in head_dptest_res:
            head_dptest_res["Force MAE"] = -1
            head_dptest_res["Force RMSE"] = -1
        ...
        OODRecord(
            run_id=f"250120_BASELINE_{model_name}",
            run_name=model_name+"#baseline#"+ood_name,
            model_version="Matbench-Discovery",
        ...
        # print(f"Finished {head_dptest_res}")


if __name__ == "__main__":
    import yaml
    with open("/mnt/data_nas/cc/LAMstare_new/lamstare/release/ood_test/OOD_DATASET_v2.yml","r") as f:
        yaml_dd =  yaml.safe_load(f)

    mapping = {k:v['filepath'] for k, v in yaml_dd["OOD_TO_HEAD_MAP"].items() if k in ["HEA25","HEA25S","HPt_NC_2022"]}
    # mapping = {k:v['filepath'] for k, v in yaml_dd["OOD_TO_HEAD_MAP"].items() if k == "ANI"}
    # mapping = {
    #     k: v
    #     for k, v in mapping.items()
    #     if k
    #     in [
    #         # "COLL_train", # RuntimeError: The expanded size of the tensor (10) must match the existing size (11) at non-singleton dimension 0.  Target sizes: [10].  Tensor sizes: [11]
    #         # "H_nature_2022", # RuntimeError: The expanded size of the tensor (2) must match the existing size (3) at non-singleton dimension 0.  Target sizes: [2].  Tensor sizes: [3]
    #         "CGM_MLP_NC2023",  # RuntimeError: The expanded size of the tensor (146) must match the existing size (147) at non-singleton dimension 0.  Target sizes: [146].  Tensor sizes: [147]
    #         # "BiarylDrugFragments",
    #         # "genentech",
    #         # "raw_torsionnet500",
    #     ]
